...
Ships `.mcp.json` registering the `wiki` stdio server (`prime`, `search` tools) through the Package launcher, `scripts/run-kmd.mjs` — a global `kmd` at or above the floor runs in-process, else `npx @bartolli/kmd@latest`. The registration carries no vault root, so the engine's full resolution chain stays live: a project vault, then `WIKI_VAULT`, then the machine default from `kmd config set default_vault <path>`. `LOG_LEVEL` passes through the plugin's `env_vars` allowlist to override the default `info`. If the `wiki` server is also registered globally, remove the global entry when enabling this plugin to avoid double registration.

**Per-project vaults.** Gate hooks are project-aware automatically — every hook event carries the session working directory, and a project carrying its own vault (co-located `vault/vault.yaml`, `.kmd`-marked root vault, or committed `.kmd/config.yaml`) resolves ahead of the configured default. The MCP server cannot see the workspace on its own (codex through 0.153.0 launches plugin MCP servers in the plugin cache directory with no workspace signal and no `roots` capability), so per-project `prime`/`search` needs an explicit export in the shell that launches codex: `export KMD_PROJECT_DIR="$PWD"` engages the same chain, or `WIKI_VAULT` pins the vault directly. Both pass through the plugin's `env_vars` whitelist. The request for a workspace signal to plugin MCP servers is open upstream as openai/codex#37903.

**Startup grace.** Codex builds its initial tool catalog after `mcp_optional_startup_grace_ms` (default 1000 ms) for servers not marked `required`; the launcher's `npx` hop can exceed it, and the session then reports the `wiki` server timed out while the hooks keep working. Until the launcher prefers a global `kmd`, set `mcp_optional_startup_grace_ms = 0` in `~/.codex/config.toml` so Codex waits for the server's own `startup_timeout_sec`.
