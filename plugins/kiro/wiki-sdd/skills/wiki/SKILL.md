...
**If the `wiki` MCP server is already registered** (workspace or user `.kiro/settings/mcp.json`): nothing to do — skip to step 7. A project vault takes the workspace-level file pointing at it (§ Harness placement). On CoCo the same export is only needed when the session was not launched from the project root.
...
- **Check for an existing wiki MCP server** (plugin-bundled or user-registered) before offering to register one. On Kiro, registration lives in `.kiro/settings/mcp.json` (workspace or user) — see `templates/mcp-entry-kiro.json.template`.
