...
**If the `wiki` MCP server is already registered** (workspace or user `.kiro/settings/mcp.json`): nothing to do — skip to step 6. A project vault takes the workspace-level file pointing at it (§ Harness placement). On CoCo the same export is only needed when the session was not launched from the project root.
