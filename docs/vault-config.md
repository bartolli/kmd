...
`vault.yaml` at the vault root is the controlled vocabulary and the served pedagogy for one vault. Loading is fail-loud: an invalid file stops the MCP server from starting and blocks `kmd sync` and `kmd validate`. Unknown keys are rejected — a typo'd field never silently does nothing.
