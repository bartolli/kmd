...
- **Everything under `kmd hook` exits 0 except a rendered deny under the kiro codec**, one stderr diagnostic on degraded paths. Exit 2 on UserPromptSubmit blocks the user's prompt — gates fail open, loudly. Under `--harness kiro` a pretool deny exits 2 with the reason on stderr, Kiro's only deny channel; every other path there stays 0. The operator surface (`sync`, `validate`) keeps real exit codes.
